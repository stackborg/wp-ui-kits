...
- **Card.tsx**: Replaced hardcoded hex colors (`#334155`, `#0f172a`) with CSS custom properties (`--sb-color-text-secondary`, `--sb-color-text`)
- **ConfirmDialog.tsx**: Added `useEffect` keyboard listener for Escape key dismiss
- **client.ts**: Removed duplicate `WordPressData` interface — now imports from `types/wordpress.d.ts`
- **bootstrap.tsx**: Removed unused `styles` parameter from `createDashboard()`

### Added

- **index.css**: `sb-pulse` and `sb-ping` keyframe animations
